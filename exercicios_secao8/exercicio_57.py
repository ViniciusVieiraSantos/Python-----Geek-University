...
x = int(input('Digite o numero da coluna (1 a 6) da matriz A a ser somada: '))
if x < 1 or x > 6:
    while x < 1 or x > 6:
        print(f'A matriz não possui a coluna {x}, digite um valor entre 1 e 6 : ')
        x = int(input())

print('Matriz A:')
imprimir_matriz(mat)
print(f'Soma dos elementos da coluna {x} : {soma_coluna(mat, x -1)}')
